...
* 提及块：正文命中当前文档的命名/别名/文档名/引用锚文本，但未真正 `((id))` 引用的内容块（与官方反链面板「提及」一致）
...
面板顶部可在「引用」与「提及」之间切换。提及用于查看正文提到了当前文档、但尚未建立真实块引用的内容。

**怎么用**

* 点击顶部「引用 / 提及」切换范围；两种模式的筛选条件各自独立保存，切换不会互相覆盖
* 提及模式下可用：关联定义块、提及所在文档、关键字、排序、分页、展开/折叠、保存条件等
* 提及关键字走服务端过滤，输入后会重新取数

**边界**

* 提及块本身不引用当前文档，因此不支持「当前文档定义块」过滤，也不支持「反链块锚文本类型」筛选；切换到提及后这两项会自动隐藏
* 关联定义块/锚文本筛选，依赖提及块上下文中真实存在的 `((id))` 引用
* 数据与官方提及一致，受思源内核虚拟引用规则约束

### （6）传递型反链：文档与标题

跟进思源 3.8 内核。原有列表项、引述块、超级块的传递不变，本次补上「文档首段」和「标题下首段」两种。

**规则**（与官方一致）

1. 文档下**首个块**是仅含块引用的段落 → 反链单位是**整篇文档**
2. 标题下**首个块**是仅含块引用的段落 → 反链单位是**该标题及其下属全部内容**
3. 段落里还有正文、图片、标签等 → 不传递
4. 同一文档已按规则 1 传递 → 文内标题不再按规则 2 传递；标题嵌套时保留覆盖范围更大的那个

**功能表现**

命中后这条反链按「文档」或「标题」展示，不再只显示那一行引用；该单位下属的其它引用合并进这一条，一篇文档 / 一个标题只保留一条。关键字、关联定义块、筛选的查询范围跟着扩大：
...
设置里「查询标题下关联定义块」的语义不变，仍只管「引用写在标题文字里」这个旧场景；标题下首段传递时**始终**按标题下属全文查询，不依赖该开关。

**边界**

* 父级是列表项时仍走列表项传递，不升级成文档 / 标题
* 提及模式不受影响
* 展示与排序仍以反链块为单位，不改成官方的按文档分组
* 无开关，默认与官方一致；内核低于 3.8.0 时不启用（旧内核不做这层传递，跟着合并反而会丢条目）
